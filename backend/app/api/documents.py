...
@router.post(
    ...
    status_code=status.HTTP_200_OK,
    summary="Multi-File Bulk Upload and Auto-Ingest",
    description="Upload multiple PDF/DOCX documents at once, classify them, auto-ingest into vector store, and return per-file results.",
)
def bulk_upload_documents(
    ...
    """Handle multi-file upload and automatic knowledge base ingestion."""
    logger.info("Received bulk upload request for %d file(s) (classification='%s')", len(files), classification)

    if not files:
        raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail="No files were provided for bulk upload.")

    clean_classif = classification.strip().lower()
    if clean_classif not in VALID_CLASSIFICATIONS:
        raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail="Classification must be 'public' or 'confidential'.")

    ingestion_service: IngestionService = getattr(request.app.state, "ingestion_service", None)
    ...
    max_bytes = 20 * 1024 * 1024  # 20MB limit per file
    ...
    for file in files:
        ...
        if ext not in ALLOWED_EXTENSIONS:
            results.append({
                ...
                "status": "failed",
                "chunks": 0,
                "error": f"Unsupported extension '{ext}'. Only .pdf and .docx supported.",
            })
            failed += 1
            continue
        ...
        try:
            ...
            # Save temporary file
            target_dir = Path("Documents") / clean_classif
            ...
            # Ingest
            ingest_res = ingestion_service.ingest(temp_path, classification=clean_classif)
            ...
            try:
                ...
                if audit_logger:
                    audit_logger.log_event(
                        question=f"UPLOAD /api/documents/bulk-upload '{filename}'",
                        decision="SYSTEM_ACTION",
                        severity="LOW",
                        categories=["DOCUMENT_MANAGEMENT"],
                        reason=f"Ingested '{filename}' ({clean_classif}) into vector store ({ingest_res.get('chunks', 0)} chunks).",
                        matched_document=filename,
                        confidence=1.0,
                    )
            except Exception:
            ...
            logger.error("Failed to upload/ingest '%s': %s", filename, exc)
            results.append({
                "filename": filename,
                "status": "failed",
                "chunks": 0,
